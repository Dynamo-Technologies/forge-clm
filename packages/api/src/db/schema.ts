...
export const contracts = contractsSchema.table(
  "contracts",
  {
    ...
    primeOrSub: varchar("prime_or_sub", { length: 20 }),
    vehicleName: varchar("vehicle_name", { length: 255 }),
    vehicleContractNumber: varchar("vehicle_contract_number", { length: 100 }),
    taskOrderNumber: varchar("task_order_number", { length: 100 }),
    placeOfPerformance: varchar("place_of_performance", { length: 500 }),
    setAsideType: varchar("set_aside_type", { length: 50 }),
    competitionType: varchar("competition_type", { length: 100 }),
    awardDate: date("award_date"),
    basePeriodEnd: date("base_period_end"),
    totalOptionValue: numeric("total_option_value", { precision: 15, scale: 2 }),
    incumbent: boolean("incumbent"),
    recompeteDate: date("recompete_date"),
    contractVehicleId: uuid("contract_vehicle_id"),
  ...
  (table) => [
    ...
    index("idx_contracts_vehicle_id").on(table.contractVehicleId),
    index("idx_contracts_recompete_date").on(table.recompeteDate),
  ],
);
